    ...
    <div>
      ...
      <input
        ...
        className={cn(
          ...
          props.disabled ? 'bg-ink-50 text-ink-400 border-ink-200 cursor-not-allowed' :
          error   ? 'border-red-400 bg-white' :
          focused ? 'border-brand-500 bg-white ring-2 ring-brand-500/16' :
...
export default function SettingsPage() {
  ...
  const onProfileSave = (_: ProfileForm) =>
    showToast({ title: '프로필이 저장되었습니다', kind: 'success' })

  const onPwSave = (_: PwForm) => {
    showToast({ title: '비밀번호가 변경되었습니다', kind: 'success' })
    pwForm.reset()
  }
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-4 mb-6 pb-6 border-b border-ink-100">
                ...
                <div>
                  ...
                  <button className="mt-2 text-[12px] text-brand-600 font-semibold hover:text-brand-800">
                    사진 변경
                  </button>
                </div>
              </div>

              <form onSubmit={profileForm.handleSubmit(onProfileSave)} className="flex flex-col gap-5 max-w-[480px]">
                <InputField
                  label="이름"
        ...
        {tab === 'password' && (
          <div className="bg-white rounded-xl border border-ink-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-ink-100">
              ...
              <p className="text-[12px] text-ink-500 mt-0.5">계정 보안을 위해 주기적으로 변경해주세요</p>
            </div>
            <div className="p-6">
              <form onSubmit={pwForm.handleSubmit(onPwSave)} className="flex flex-col gap-5 max-w-[480px]">
                <InputField
                  label="현재 비밀번호"
                  type="password"
