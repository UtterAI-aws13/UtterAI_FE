...
const GENDER_LABEL: Record<string, string> = { M: '남성', F: '여성', U: '미입력' }
...
export default function PatientsPage() {
  ...
  const filtered = patients.filter((p) => {
    ...
    if (filter === 'new') {
      if (new Date(p.created_at) < new Date(Date.now() - 90 * 24 * 60 * 60 * 1000)) return false
    }
    return true
  })

  const handleDelete = async (p: Patient, e: React.MouseEvent) => {
    e.stopPropagation()
    try {
      await patientsApi.delete(p.id)
      setPatients((ps) => ps.filter((x) => x.id !== p.id))
      showToast({ title: '환자 정보가 삭제되었습니다', body: `${p.name} 환자 정보가 삭제되었어요.`, kind: 'success' })
    } catch {
  ...
  return (
    <div>
      <div className="px-8 pt-7 pb-5 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-ink-800 tracking-tight">환자 관리</h1>
          <p className="text-[13px] text-ink-500 mt-1">총 {patients.length}명의 환자</p>
        </div>
        <button
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-[15px] font-semibold text-ink-800">
                {query ? '검색 결과가 없습니다' : '아직 등록된 환자가 없습니다'}
              </p>
              <p className="text-[12px] text-ink-500 mt-1.5">
                {query ? '다른 이름으로 다시 시도해보세요.' : '첫 환자를 등록하면 세션 분석을 시작할 수 있어요.'}
              </p>
            </div>
          ) : (
                  ...
                  >
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        <div className={avatarClass(p.gender)}>{p.name[0]}</div>
                        <div>
                          <p className="font-semibold text-ink-800">{p.name}</p>
                          <p className="text-[11px] text-ink-500 mt-0.5">{computeAge(p.birth_date)}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3 font-mono-num text-ink-500">{formatDate(p.birth_date)}</td>
                    <td className="px-5 py-3">
                      <span className={genderChipClass(p.gender)}>{GENDER_LABEL[p.gender ?? 'U'] ?? '미입력'}</span>
                    </td>
                    <td className="px-5 py-3 text-ink-500 max-w-[180px] truncate">{p.memo ?? '—'}</td>
                    <td className="px-5 py-3 font-mono-num text-ink-500">{formatDate(p.created_at)}</td>
      ...
      {showModal && (
        <PatientFormModal
          ...
          onDone={(patient) => setPatients((ps) => [patient, ...ps])}
        />
      )}
    </div>
  )
